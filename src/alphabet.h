...
struct FlashPattern convertString(char inputString[], uint8_t letterKerning){
	...
	for(uint16_t i = 0; i < strlen(inputString); i++)
	{
		...
		if((int)Alphabet[(int)inputString[i]] == 32) // Check if [space]
		{
			for(uint16_t j = 0; j < 5; j++)  // Iterate through each character's value
			{
				data[(5+letterKerning)*i + j] = Alphabet[26][j]; 
			}
		} 
		else                                   // Assign letter (starting at ASKII 65)
		{
			for(int j = 0; j < 5; j++)
			{
				data[(5+letterKerning)*i + j] = Alphabet[inputString[i] - 65][j];
			}
		}
